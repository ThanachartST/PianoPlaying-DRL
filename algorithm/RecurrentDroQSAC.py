...
class RecurrentDroQSACConfig:
    ...
    fc_hidden_sizes: Sequence[int] = (256, 256, 256)    # Networks hidden sizes
...
class RecurrentDroQSACAgent(object):
    '''
    ...
    def __init__(self, 
        ...
        seq_obs_dim = spec.seq_obs.shape[-1]
        static_obs_dim = spec.static_obs.shape[-1]

        # Intitialize temperature
        alpha = config.init_temperature

        # Set up policy network
        self.policy_net = RecurrentTanhGaussianPolicy(seq_obs_dim = seq_obs_dim,
                                                      static_obs_dim = static_obs_dim,
                                                      action_dim = act_dim,
                                                      rnn_hidden_size = config.rnn_hidden_size,
                                                      fc_hidden_sizes = config.fc_hidden_sizes).to(config.device)

        # Set up q networks
        self.q_net_list, self.q_target_net_list = [], []
        for q_i in range(config.num_Q):
            # Declare Q-networks
            new_q_net = RnnMlp(seq_obs_dim = seq_obs_dim, 
                               static_obs_dim = static_obs_dim + act_dim,
                               output_size = 1, 
                               rnn_hidden_size = config.rnn_hidden_size, 
                               fc_hidden_sizes = config.fc_hidden_sizes, 
                               target_drop_rate = config.critic_dropout_rate, 
                               layer_norm = config.critic_layer_norm).to(config.device)
            self.q_net_list.append(new_q_net)
            # Decalre target Q-networks, load the same parameters as main Q-netwokrs
            new_q_target_net = RnnMlp(seq_obs_dim = seq_obs_dim, 
                                      static_obs_dim = static_obs_dim + act_dim,
                                      output_size = 1, 
                                      rnn_hidden_size = config.rnn_hidden_size, 
                                      fc_hidden_sizes = config.fc_hidden_sizes, 
    ...
    def sample_actions(self, 
        ...
        with torch.no_grad():
            seq_data = obs[self.seq_obs_spec.name]
            static_data = obs[self.static_obs_spec.name]

            # Convert observation in array form to Tensor form
            seq_obs_tensor = torch.Tensor(seq_data).unsqueeze(0).to(self.device)
            static_obs_tensor = torch.Tensor(static_data).unsqueeze(0).to(self.device)
            # Sample action in Tensor form
            action_tensor = self.policy_net.forward(seq_input = seq_obs_tensor,
                                                    static_input = static_obs_tensor, 
                                                    deterministic = False,
    ...
    def get_droq_q_target_no_grad(self, 
                                  next_seq_obs_tensor: Tensor,
                                  next_static_obs_tensor: Tensor, 
        ...
        with torch.no_grad():
            if self.q_target_mode == 'min':
                ...
                a_tilda_next, _, _, log_prob_a_tilda_next, _, _ = self.policy_net.forward(seq_input = next_seq_obs_tensor,
                                                                                          static_input = next_static_obs_tensor)

                # Compute Q values in the next timestep
                q_prediction_next_list = []
                for sample_idx in range(self.num_Q):
                    q_prediction_next = self.q_target_net_list[sample_idx](next_seq_obs_tensor, torch.cat([next_static_obs_tensor, a_tilda_next], 1))
    ...
    def update_actor(self, 
                     transitions: RecurrentTransitionTensor):
        ''' 
        ...
        a_tilda, _, _, log_prob_a_tilda, _, _ = self.policy_net.forward(seq_input = transitions.seq_state,
                                                                        static_input = transitions.static_state)

        # Compute Q-values from all Q networks
        q_a_tilda_list = []
        for sample_idx in range(self.num_Q):
            self.q_net_list[sample_idx].requires_grad_(False)
            q_a_tilda = self.q_net_list[sample_idx](transitions.seq_state, torch.cat([transitions.static_state, a_tilda], 1))
    ...
    def update_critic(self, 
        ...
        y_q = self.get_droq_q_target_no_grad(next_seq_obs_tensor = transitions.next_seq_state,
                                             next_static_obs_tensor = transitions.next_static_state,
                                             reward_tensor = transitions.reward,
                                             done = 0)

        # Compute Q values from all Q networks
        q_prediction_list = []
        for q_i in range(self.num_Q):
            q_prediction = self.q_net_list[q_i](transitions.seq_state, torch.cat([transitions.static_state, transitions.action], 1))
